<x-layouts.app :title="'Enter Scores'">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-neutral-900 dark:text-white mt-2">Enter Scores</h1>
        <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">Enter CA and Examination scores for your students</p>
    </div>
    ...
    <form id="scores-form" method="POST" action="{{ route('teacher.scores.store') }}">
        ...
        <div class="space-y-6">
            ...
            <x-ui.card>
                <div class="px-4 py-3 border-b border-neutral-200 dark:border-dark-border">
                    <h3 class="text-sm font-semibold text-neutral-900 dark:text-white">{{ $assignment->classSubject->subject->name ?? 'Unknown' }}</h3>
                </div>
                <div class="p-4">
                        ...
                        <table class="w-full">
                            <thead class="bg-neutral-50 dark:bg-neutral-800">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">CA Score</th>
                                    <th class="px-4 py-3 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Exam Score</th>
                            ...
                            <tbody>
                                ...
                                @php 
                                $result = $student->results->where('term_id', $classAssignment->term_id)->where('class_subject_id', $assignment->id)->first();
                                @endphp
                                <tr class="border-b border-neutral-100 dark:border-neutral-800 last:border-0">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <input type="number" 
                                               ...
                                               max="100" 
                                               step="0.5"
                                               name="results[{{ $assignment->id }}][{{ $student->id }}][ca_score]" 
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <input type="number" 
                                               ...
                                               max="100" 
                                               step="0.5"
                                               name="results[{{ $assignment->id }}][{{ $student->id }}][exam_score]" 
                                               class="w-16 px-2 py-1 border border-neutral-300 dark:border-dark-border rounded-lg text-sm text-center bg-neutral-50 dark:bg-neutral-800"
        ...
        <div class="px-4 py-3 border-t border-neutral-200 dark:border-dark-border flex gap-3 justify-center">
    ...
    <x-ui.card>
        <div class="p-6 text-center">
            <p class="text-neutral-500 dark:text-neutral-400 mb-4">No students found in your class for the current term.</p>
        </div>
    </x-ui.card>
    @endif
</x-layouts.app>