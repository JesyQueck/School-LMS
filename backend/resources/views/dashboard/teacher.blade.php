    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        @if($classAssignment)
            <x-ui.stat-card
                ...
                :trend="['direction' => 'neutral', 'value' => 'Class Teacher']"
                icon="school"
            />
        @else
            <x-ui.stat-card
                label="Attendance"
                value="96%"
                :trend="['direction' => 'up', 'value' => '1.2% from last week']"
                icon="calendar-check"
            />
        @endif
    </div>
                                        ...
                                        <div id="tracker-summary" class="mt-4 pt-3 border-t border-neutral-200 dark:border-dark-border">
                                            ...
                                            <div id="tracker-submit-btn" class="mt-3 hidden">
                                                <a
                                                    ...
                                                    class="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors text-center block"
                                                >
                                ...
                                <div class="flex items-start gap-3 p-3 rounded-lg border border-neutral-200 dark:border-dark-border">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-neutral-900 dark:text-white">
                                            {{ $assignment->classSubject->subject->name ?? 'Unknown' }}
                                        </p>
            ...
            <x-ui.card>
                <div class="px-6 py-5 border-b-2 border-neutral-200 dark:border-dark-border">
                    ...
                    <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
                        Verify all requirements before submitting report cards
                    </p>
                </div>
                ...
                <div class="p-4" id="completion-checklist">
                    ...
                    <div id="checklist-content" class="hidden">
                        ...
                        <div
                            id="checklist-submit-section"
                            class="mt-4 pt-3 border-t border-neutral-200 dark:border-dark-border text-center hidden"
                        >
                            <a
                                href="{{ route('teacher.report-cards.index') }}"
                                id="checklist-submit-btn"
                                class="inline-block px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors"
                            >
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (trackerEl) {
                        ...
                        .then(data => {
                            ...
                            document.getElementById('tracker-progress').textContent =
                                data.all_scores_submitted
                                    ? 'All scores submitted!'
                                    : (data.total_subjects - data.submission_progress.filter(p => !p.completed).length) +
                                      '/' +
                                      data.total_subjects +
                                      ' subjects submitted';
                    ...
                    if (checklistEl) {
                        ...
                        .then(data => {
                            ...
                            items += '<li class="flex items-center gap-2"><span class="text-green-500">' +
                                (data.all_scores_submitted ? '✔' : '❌') +
                                '</span> All subject scores submitted</li>';

                            items += '<li class="flex items-center gap-2"><span class="text-green-500">' +
                                (data.attendance_submitted ? '✔' : '❌') +
                                '</span> Attendance generated</li>';

                            items += '<li class="flex items-center gap-2"><span class="text-green-500">' +
                                (data.comments_completed ? '✔' : '❌') +
                                '</span> Grades calculated</li>';

                            items += '<li class="flex items-center gap-2"><span class="text-green-500">' +
                                (data.comments_completed ? '✔' : '❌') +
                                '</span> Average calculated</li>';

                            items += '<li class="flex items-center gap-2"><span class="text-green-500">' +
                                (data.comments_completed ? '✔' : '❌') +
                                '</span> Affective Domain completed</li>';

                            items += '<li class="flex items-center gap-2"><span class="text-green-500">' +
                                (data.comments_completed ? '✔' : '❌') +
                                '</span> Psychomotor Assessment completed</li>';

                            items += '<li class="flex items-center gap-2"><span class="text-green-500">' +
                                (data.comments_completed ? '✔' : '❌') +
                                '</span> Teacher Comment entered</li>';

                            document.getElementById('checklist-items').innerHTML = items;

                            if (data.is_ready_to_submit) {
                                document
                                    .getElementById('checklist-submit-section')
