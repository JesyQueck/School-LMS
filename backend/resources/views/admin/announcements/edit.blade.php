<x-layouts.app title="Edit Announcement">
    <div class="mb-6">
        <x-ui.breadcrumbs>
            ...
            <x-ui.breadcrumb-item active>Edit Announcement</x-ui.breadcrumb-item>
        </x-ui.breadcrumbs>
        <h1 class="text-2xl font-bold text-neutral-900 dark:text-white mt-2">Edit Announcement</h1>
        <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">Update announcement details.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.announcements.update', $announcement) }}" enctype="multipart/form-data" class="space-y-6" x-data="{ isEvent: {{ $announcement->is_event ? 'true' : 'false' }} }">
        @csrf
        @method('PATCH')
        ...
        <x-ui.card>
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="body" name="body" rows="6" required class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text placeholder-neutral-400 dark:placeholder-neutral-500 px-3 py-2 text-base transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Write your announcement message here...">{{ old('body', $announcement->body) }}</textarea>
                    @error('body')
                        <p class="text-sm text-danger-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if ($announcement->image)
                        <div id="current-image" class="mt-3">
                            <img src="{{ asset('storage/' . $announcement->image) }}" alt="Current" class="max-w-full max-h-48 rounded-lg border border-neutral-300 dark:border-dark-border object-cover">
                            <div class="flex items-center gap-2 mt-2">
                                <input type="checkbox" name="clear_image" id="clear_image" value="1" class="h-4 w-4 rounded border-neutral-300 dark:border-dark-border text-danger-600 focus:ring-danger-500 dark:bg-dark-surface">
                                <label for="clear_image" class="text-sm text-neutral-600 dark:text-neutral-400">Remove current image</label>
                            </div>
                        </div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <select id="target_role" name="target_role" required class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-base transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 appearance-none">
                        ...
                        <option value="all" {{ old('target_role', $announcement->target_role) == 'all' ? 'selected' : '' }}>All Users (Students, Teachers & Parents)</option>
                        ...
                        <option value="parent" {{ old('target_role', $announcement->target_role) == 'parent' ? 'selected' : '' }}>Parents Only</option>
                    </select>
                    @error('target_role')
                        <p class="text-sm text-danger-500 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-start pt-2">
                    <div class="flex items-center h-5">
                        <input id="show_on_website" name="show_on_website" type="checkbox" value="1" {{ old('show_on_website', $announcement->show_on_website) ? 'checked' : '' }} class="h-4 w-4 rounded border-neutral-300 dark:border-dark-border text-primary-600 focus:ring-primary-500 dark:bg-dark-surface">
                    </div>
                    <div class="ml-2">
                ...
                <div class="pt-2 border-t border-neutral-200 dark:border-dark-border">
                    <div class="flex items-start">
                        <div class="flex items-center h-5 mt-0.5">
                            <input id="is_event" name="is_event" type="checkbox" value="1" {{ old('is_event', $announcement->is_event) ? 'checked' : '' }} class="h-4 w-4 rounded border-neutral-300 dark:border-dark-border text-primary-600 focus:ring-primary-500 dark:bg-dark-surface" x-model="isEvent">
                        </div>
                        <div class="ml-2">
                ...
                <div x-show="isEvent" class="grid grid-cols-1 md:grid-cols-2 gap-4" x-cloak>
                    <div>
                        ...
                        <input id="event_date" name="event_date" type="date" value="{{ old('event_date', $announcement->event_date?->format('Y-m-d')) }}" class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-primary-500">
                        @error('event_date')
                            <p class="text-sm text-danger-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input id="event_time" name="event_time" type="time" value="{{ old('event_time', $announcement->event_time?->format('H:i')) }}" class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-primary-500">
                        @error('event_time')
                            <p class="text-sm text-danger-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="flex gap-3 justify-end">
            ...
            <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-medium px-4 py-2 rounded-lg shadow-sm transition-colors text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-dark-bg">Update Announcement</button>
        </div>
    </form>
</x-layouts.app>
