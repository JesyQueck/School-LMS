...
@endphp
<div class="grid grid-cols-5 gap-1.5 items-end py-1">
    ...
    <div>
        ...
        <input type="time" name="periods[{{ $index }}][start_time]" value="{{ \Carbon\Carbon::parse($period->start_time)->format('H:i') }}" class="w-24 rounded border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-1.5 py-0.5 text-xs">
    </div>
    <div>
        <label class="block text-xs text-neutral-500 dark:text-neutral-400 mb-0.5">End</label>
        <input type="time" name="periods[{{ $index }}][end_time]" value="{{ \Carbon\Carbon::parse($period->end_time)->format('H:i') }}" class="w-24 rounded border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-1.5 py-0.5 text-xs">
    </div>
