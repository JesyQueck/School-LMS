<x-layouts.app title="Add Timetable Entry">
    <x-ui.breadcrumbs>
        ...
        <x-ui.breadcrumb-item active>Add Entry</x-ui.breadcrumb-item>
    </x-ui.breadcrumbs>

    <div class="mb-6">
        <h1 class="text-2xl font-bold text-neutral-900 dark:text-white mt-2">Add Timetable Entry</h1>
        <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">Create a new class schedule entry.</p>
    </div>
    ...
    <x-ui.card>
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('admin.timetable.store') }}" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                @csrf
                        ...
                        @foreach($classes as $class)
                            <optgroup label="{{ $class->name }}">
                                @foreach($class->classSubjects as $cs)
                                    <option value="{{ $cs->id }}" {{ old('class_subject_id') == $cs->id ? 'selected' : '' }}>{{ $cs->subject->name ?? 'Unknown Subject' }}</option>
                                @endforeach
                            </optgroup>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="time" name="start_time" value="{{ old('start_time') }}" required class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary-500">
                    @error('start_time')
                        <p class="mt-1 text-xs text-danger-600 dark:text-danger-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="time" name="end_time" value="{{ old('end_time') }}" required class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary-500">
                    @error('end_time')
                        <p class="mt-1 text-xs text-danger-600 dark:text-danger-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="teacher_id" class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary-500">
                        ...
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('teacher_id') == $teacher->id ? 'selected' : '' }}>{{ $teacher->user?->name ?? 'Unnamed Teacher' }}</option>
                        @endforeach
                    </select>
                    @error('teacher_id')
                        <p class="mt-1 text-xs text-danger-600 dark:text-danger-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="academic_session_id" class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary-500">
                        ...
                        @foreach($sessions as $session)
                            <option value="{{ $session->id }}" {{ old('academic_session_id', $currentTerm?->academic_session_id) == $session->id ? 'selected' : '' }}>{{ $session->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="term_id" class="w-full rounded-lg border border-neutral-300 dark:border-dark-border bg-white dark:bg-dark-surface text-neutral-900 dark:text-dark-text px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary-500">
                        ...
                        @foreach($terms as $term)
                            <option value="{{ $term->id }}" {{ old('term_id', $currentTerm?->id) == $term->id ? 'selected' : '' }}>{{ $term->name }}</option>
                        @endforeach
                    </select>
                    @error('term_id')
                        <p class="mt-1 text-xs text-danger-600 dark:text-danger-400">{{ $message }}</p>
                ...
                <div class="sm:col-span-2 flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-medium px-4 py-2 rounded-lg shadow-sm">Save Entry</button>
                </div>
            </form>
        </div>
    </x-ui.card>
